...
def make_param(shape_list):
    ...
    for i in range(len(shape_list) - 1):
        w = cp.random.randn(shape_list[i], shape_list[i + 1])
        b = cp.ones(shape_list[i + 1]) / 10
        w_list.append(w)
        b_list.append(b)
...
def calculate(x, w_list, b_list, y):  # ++
    ...
    y2 /= cp.sum(y2, axis = 1, keepdims=True)
...
def update(x_train, w_list, b_list, y_train, lr):
    ...
    b_list[1] -= lr * cp.sum(d12_d8 * d8_d7, axis = 0)
    w_list[1] -= lr * cp.dot(d8_d6, d12_d8)
    
    #d12_d8 = d12_d11 * d11_d9 * d9_d8
    d12_d5 = cp.dot(d12_d8, d8_d5)
    d12_d4 = d12_d5 * d5_d4
    b_list[0] -= lr * cp.sum(d12_d4 * d4_d3, axis = 0)
    w_list[0] -= lr * cp.dot(d4_d2, d12_d4)
...
def accuracy(x, w_list, b_list, y):
    ...
    predict = val_dict['y2']
    result = cp.where(cp.argmax(y, axis=1) == cp.argmax(predict, axis=1), 1, 0).mean()
    return result
